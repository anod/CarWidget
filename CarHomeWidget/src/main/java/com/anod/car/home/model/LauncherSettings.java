	...
	public static final class Favorites implements BaseColumns {
		...
		static Uri getContentUri(String packageName) {
			if (Version.isFreeVersion(packageName)) {
				return Uri.parse(CONTENT_PREFIX + LauncherProvider.AUTHORITY_FREE + "/" + LauncherProvider.TABLE_FAVORITES);
			}
			return Uri.parse(CONTENT_PREFIX + LauncherProvider.AUTHORITY_PRO + "/" + LauncherProvider.TABLE_FAVORITES);
		}
		...
		static Uri getContentUri(String packageName, long id) {
			if (Version.isFreeVersion(packageName)) {
				return Uri.parse(CONTENT_PREFIX + LauncherProvider.AUTHORITY_FREE + "/" + LauncherProvider.TABLE_FAVORITES + "/" + id);
			}
			return Uri.parse(CONTENT_PREFIX + LauncherProvider.AUTHORITY_PRO + "/" + LauncherProvider.TABLE_FAVORITES + "/" + id);
		}
