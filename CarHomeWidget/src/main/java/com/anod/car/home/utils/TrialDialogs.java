...
public class TrialDialogs {
	...
	public static Dialog buildTrialDialog(int trialsLeft,final Context context) {
		...
		int negativeRes = 0;
		if (trialsLeft > 0) {
			String activationsLeftText = context.getResources().getQuantityString(R.plurals.notif_activations_left, trialsLeft, trialsLeft);

			builder.setTitle(R.string.dialog_donate_title_trial);
			builder.setMessage(activationsLeftText + context.getString(R.string.dialog_donate_message_trial));
	...
	private static void setTrialDialogButtons(final Context context, AlertDialog.Builder builder, int negativeRes) {
